...
#delete prior data
for i in range(0,60):
	cursor.execute("DELETE FROM `JPY` WHERE No = '%d'"% (i))
...
for i in range(0,60):
	...
	list1.append(sell)
	try:
		cursor.execute('INSERT INTO JPY(No,Date,Buy,Sell) VALUES({},"{}",{},{})'.format(i,date,buy,sell))
		db.commit()
	except:
		db.rollback()
...
#delete prior data
cursor.execute("DELETE FROM `range_recommend` WHERE Currency = 'JPY'")

# update database
try:
	cursor.execute('INSERT INTO range_recommend(No,Currency,Recommend,Min,Max) VALUES({},"{}",{},{},{})'.format(7,'JPY',recommend,list1[0],list1[59]))
	db.commit()
except:
	print('error')
	db.rollback()
